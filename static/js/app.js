...
//----ANIMATED HEADER---//
function stickyNav(){
    var headerHeight = document.querySelector("#nvbar").offsetHeight / 2;
...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function() {
    ...
    function() {
        function t() {
            ...
            for (var r in d) {
                var T = new l(d[r], 2 + 2 * Math.random(), "rgba(118,171,174,0.3)");
                d[r].circle = T
            }
        }
        ...
        function h(t) {
            if (t.active)
                for (var e in t.closest) m.beginPath(), m.moveTo(t.x, t.y), m.lineTo(t.closest[e].x, t.closest[e].y), m.strokeStyle = "rgba(118,171,174," + t.active + ")", m.stroke()
        }
